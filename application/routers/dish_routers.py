...
async def create_dish(target_menu_id: int, target_submenu_id: int, schema: schemas.DishCreate,
                      dish: DishService = Depends()) -> HTTPException | schemas.Dish:
    return await dish.create_dish(dish_schemas=schema, menu_id=target_menu_id, submenu_id=target_submenu_id)


@router.get('/{target_menu_id}/submenus/{target_submenu_id}/dishes', response_model=list[schemas.Dish])
async def read_dishes(target_menu_id: int, target_submenu_id: int,
                      dish: DishService = Depends()) -> list[schemas.Dish]:
    return await dish.get_dishes(menu_id=target_menu_id, submenu_id=target_submenu_id)


@router.patch('/{target_menu_id}/submenus/{target_submenu_id}/dishes/{target_dish_id}',
              response_model=schemas.Dish)
async def update_dish(target_menu_id: int, target_submenu_id: int, target_dish_id: int,
                      schema: schemas.DishUpdate, dish: DishService = Depends()) -> HTTPException | schemas.Dish:
    return await dish.update_dish(dish_schemas=schema, menu_id=target_menu_id, submenu_id=target_submenu_id,
...
async def delete_dish(target_menu_id: int, target_submenu_id: int, target_dish_id: int,
    ...
    return await dish.delete_dish(menu_id=target_menu_id, submenu_id=target_submenu_id, dish_id=target_dish_id)


@router.get('/{target_menu_id}/submenus/{target_submenu_id}/dishes/{target_dish_id}',
            response_model=schemas.Dish)
async def read_dish(target_menu_id: int, target_submenu_id: int, target_dish_id: int,
                    dish: DishService = Depends()) -> HTTPException | schemas.Dish:
    return await dish.get_dish(menu_id=target_menu_id, submenu_id=target_submenu_id, dish_id=target_dish_id)
